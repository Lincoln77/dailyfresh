...
class OrderCommitView1(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            # todo: 用户的订单中有几个商品，就需要向df_order_goods表中加几条记录
            conn = get_redis_connection('default')
            cart_key = 'cart_%d' % user.id
        ...
        # todo: 清除用户购物车中对应的记录
        conn.hdel(cart_key, *sku_ids)  # 将列表拆包
                ...
                for i in range(3):
                    ...
                    if res == 0:
                        if i == 2:
                            # 尝试到第三次
                            transaction.savepoint_rollback(save_point)
                            return JsonResponse({'res': 7, 'errmsg': '订单创建失败2'})
                        continue
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        total_pay = order.total_price+order.transit_price  # 类型：Decimal
